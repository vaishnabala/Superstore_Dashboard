...
st.title(":bar_chart: Sample SupterStore EDA")
...
#create sidebar allows option to select by region
...
if not region and not state and not city:
    ...
    filtered_df = df3[df["Region"].isin(region) & df["State"].isin(state) & df["City"].isin(city)]
...
with col2:
    ...
    st.plotly_chart(fig,use_container_wight = True)
...
with cl1:
    with st.expander("Category_ViewDate"):
        ...
        st.download_button("Download Date",data = csv,file_name = "Category.csv", mime = "text/csv", help = "Click here to download the data as a CSV file")
...
with cl2:
    with st.expander("Region_ViewDate"):
        ...
        st.download_button("Download Date",data = csv, file_name = "Region.csv", mime = "text/csv", help = "Click here to download the data as a CSV file")
...
with st.expander("View Date of Time Series"):
    ...
    st.download_button("Download Date",data = csv,file_name = "Time_series.csv", mime = "text/csv", help = "Click here to download the data as a CSV file")
